...
void connmgr_listen(int port_number, sbuffer_t *buffer)
{
    ...
    if (tcp_passive_open(&listeningSocket, port_number) != TCP_NO_ERROR)
    {
        exit(EXIT_FAILURE);
    }
    ...
    while (active)
    {
        ...
        if (pollStatus == 0)
        {
            log_printf("No sensor data received. Closing TCP Connection...\n");
            active = false;
        }
        else
        {
            for (int i = 0; i < dpl_size(sockets); ++i)
            {
                ...
                {
                    log_printf("Sensor with ID %d has timed out.\n", currentSocket->sensor_id);
                ...
                else if ((fileDescriptor[i].revents & POLLIN) != 0)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        new_socket->firstTime = 1;
                        dpl_insert_at_index(sockets, new_socket, dpl_size(sockets), false);
                    }
                    else
                    {
                        // If connection is closed sensorNodeId is lost. So for reporting connection close I am saving it.
                        int sensorNodeId = currentSocket->sensor_id;
                        ...
                        // Sensor node is only assigned after the first receive call. Checking if this is the first time if 
                        // this sensors has data. If so confirming connection with a log message.
                        if (currentSocket->firstTime == 1)
                        {
                            log_printf("A sensor node with %d has opened a new connection\n", currentSocket->sensor_id);
                            currentSocket->firstTime = 0;
                        }
                        ...
                        if (result == TCP_NO_ERROR)
                        {
                            ...
                            if (sbuffer_insert(buffer, &incomingData) == SBUFFER_FAILURE)
                            {
                                active = false;
                            }                            
                        }
